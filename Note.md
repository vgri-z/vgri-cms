...
1. 公用的代码逻辑可以进行抽离，单独封装，对于每一个页面独有的逻辑就在各自的页面里面进行，将逻辑放进一个行数里面，通过参数的形式传递到hooks里面，当前页面需要就在hooks里面执行，不需要就不执行

### 针对数据的获取比页面渲染慢导致交互时数据无法展示的问题解决

通过computed函数对数据进行一个监听，让template组件绑定computed返回的可响应式对象，当数据发生变化的时候，组件就会重新加载
...
4. 01:12:00 - 01:46:46 商品信息页面 跨组件动态插槽
...
5. 01:29:00 - 01:51:00 全局获取并保存部门与角色数据
6. 01:51:00 - 02:14:31 页面动态数据的刷新处理方案
7. 02:12:31 - 02:39:00 新建编辑逻辑

# lesson12

1. 00:00 - 22:00 时间少8小时问题 初始化部门角色数据问题(请求的调用位置)
2. 22:00 - 01:04:00 软件分层思想 新建角色时角色菜单与权限的处理
